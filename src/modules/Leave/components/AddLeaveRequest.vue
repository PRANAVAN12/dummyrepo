          ...
          <v-row>
            ...
            <v-col cols="8">
              <ValidationProvider
                ...
                name="Date"
              >
                <datePickComponent    @input="GetleaveConformation()" labelname="Start Date" v-model="leave.start_date" />
              </ValidationProvider>
            </v-col>
              <v-col cols="4">
                 <ValidationProvider
...
export default {
  ...
  methods: {
    ...
    async submitCheque() {
      ...
      if (isValid && (this.leave.no_of_days<=this.maxAllocate))
          {
...
this.GetleaveConformation()
    ...
    {
        let info=new Object();
        info.start_date=this.leave.start_date;
        info.no_of_days=this.leave.no_of_days;
       ...
       this.$store
        ...
        {
          if(this.leave.no_of_days>this.maxAllocate)
          {
    ...
    createLeaveRequest() {
      ...
      let leaveRequest=new Object();
       ...
       leaveRequest.from_date=this.leave.start_date;
       leaveRequest.number_of_days=this.leave.no_of_days;
      ...
      this.$store
        .dispatch(`staff/CreateLeaveRequst`, leaveRequest)
        .then((res) => {
        ...
        .catch((err) => {
          this.isLoading = false;
          this.$toast.error("Leave Request added  failed");
          this.show = false;
        });
    },
    GetLeaveTypes() {

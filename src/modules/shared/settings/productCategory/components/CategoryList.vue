    ...
    <v-container fluid class="lighten-12 content">
      <v-card class="lighten-12 card-content mb-2">
        <v-row class="ma-0 align-right">
          <TableFilters
            ...
            :columns="columns"
          ></TableFilters>
          <ImportExportMenu :permission="'Category Report'"></ImportExportMenu>
        </v-row>
      </v-card>
      <v-card class="lighten-12 card-content mt-0 pa-3">
        ...
        >
          ...
          <template v-slot:item.parentCatgory="{ item }">
            <!-- <img :src="item.image" /> -->
          ...
          <template v-slot:item.action="{ item }">
            <list-menu
              ...
              softDeletePermission="Category Soft Delete"
...
import EditCategoryForm from "./CategoryEdit";
...
export default {
  ...
  methods: {
    ...
    OpenModal() {
      this.$refs.createcategory.GetCategory();
    ...
    EditModal(data) {
      ...
      this.$refs.editcategory.GetCategory();
    ...
    GetProductCategorySingle(id) {
      this.$store
        .dispatch(`product/GetProductCategory`, { id })
        .then((res) => {
          debugger;
          this.editProductCategory = new ProductCategory().SetData(res.data);

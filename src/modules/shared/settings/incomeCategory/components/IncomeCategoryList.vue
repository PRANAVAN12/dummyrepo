...
import CategoryForm from "./IncomeCategoryCreate";
...
import EditCategoryForm from "./IncomeCategoryEdit";
...
export default {
  ...
  methods: {
    ...
    EditModal(data) {
      ...
      this.GetIncomeCategorySingle(data.id);
    },
    EditCategory($event) {
      this.EditScheduleForm = false;
   
    },
    GetIncomeCategorySingle(id) {
      this.$store
          .dispatch(`sitesetting/GetIncomeCategory`,  id )
          .then((res) => {
            debugger;
            this.editIncomeCategory = res.data

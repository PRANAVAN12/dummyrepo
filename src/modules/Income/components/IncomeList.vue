    ...
    <v-container fluid class="lighten-12 content">
      <v-card class="lighten-12 card-content mb-2">
        <v-row class="ma-0 align-right">
          ...
          <TableFilters
              :filters="[ 'search','incomeCategory','incomeFor','dateRange']"
              v-model="filter"
              :columns="columns"
          ></TableFilters>
...
import CategoryForm from "./IncomeCreate";
import { required, email, max } from "vee-validate/dist/rules";
import pagination from '../../shared/components/pagination';

import FileUpload from "@/components/base/FileUpload";
import EditCategoryForm from "./IncomeEdit";
import {
  extend,
  ValidationObserver,
  ValidationProvider,
...
export default {
  ...
  methods: {
    ...
    EditModal(data) {
      ...
      this.GetIncomeBySingle(data.id);
    },
    EditCategory($event) {
    ...
    GetIncomeBySingle(id) {
      debugger
      this.$store
          .dispatch(`warehouse/GetIncomes`,  id )
          .then((res) => {
            debugger;
            this.Incomes = res.data
            this.Incomes.date=moment(res.data.date).format("YYYY-MM-DD")

...
export default {
  ...
  methods: {
      ...
      if (isValid) {
        ...
        if (this.selectedProductsToTransfer.length != 0) {
          ...
          if(Credentials.items.length>0 && !error)
          {
            ...
            this.$store
                .dispatch("product/TranferProductToShopFromWarehouseEdit", Credentials)
                .then((res) => {
    ...
    MapStoreTranfer(stockTransferDetails) {
      ...
      this.searchWarehouse=stockTransferDetails.wareHouse.name;
      this.StockTransfer.from_ware_house_id=stockTransferDetails.from_ware_house_id;
      this.searchShop=stockTransferDetails.shop.name;
      this.StockTransfer.to_shop_id=stockTransferDetails.to_shop_id;
      ...
      stockTransferDetails.transferItems.map(p=>{
        ...
        item.quantity=p.quantity;
        item.transferUnit=[];
        item.units=p.product.units;
        count++;
        item.returnQuantity=p.return_quantity;
        item.productUnits=[];
        item.maxReturnError=[];
        item.units.map(u=>{
          let pUnit=new Object();
          pUnit.name=u.name;
          pUnit.id=u.id;
          let unitFound=p.transferItemUnits.find(k=>k.unit.id==u.id);
          pUnit.quantity=unitFound?unitFound.quantity:0;
          pUnit.transferPecentage=unitFound? parseInt(unitFound.transferable_percentage):0, pUnit.is_changes_unit=unitFound?unitFound.is_changes_unit:false;
          item.productUnits.push(pUnit)
        })

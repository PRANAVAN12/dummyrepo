...
export default {
  ...
  methods: {
    async submit() {
      ...
      if (
        isValid
        // this.Customer.CreditDetails.maximum_credit_amount > 0 &&
        // this.Customer.CreditDetails.payment_team <= 0
      ) {
        this.CreateCustomer();
      } else if (isValid) {
      }
    ...
    CreateCustomer() {
      ...
      this.$store
        .dispatch("customer/CreateCustomer", this.Customer.toRequest())
        .then(async(res) => {
            if (this.selectedImages) {
            await this.UploadCustomerImage(res.data.id)
              .then(() => {
    ...
    async UploadCustomerImage(CustomerId) {
      ...
      await this.$store.dispatch(`customer/UploadCustomerImage`, fromData);
    },
    ...
    GetSiteSetting() {
      this.$store
          ...
          .then((res) => {
            ...
            this.Customer.customer_type_id=self.siteSetting.customerType.id;
            this.Customer.customer_category_id=self.siteSetting.customerGroup.id;

                  ...
                  <v-col cols="12">
                    ...
                    >
                      ...
                      <template v-slot:item.amount="{ item }">
                        {{ item.sub_total_amount + item.additional_amount }}
                      </template>
                      <template v-slot:item.exchange="{ item }">
                        {{ item.is_exchange ? "Exchange" : "Cash" }}
                      </template>

                      <template v-slot:footer="{}">
                        <paginationComponent
                          @paginationOptions="setPaginationOptions"
                          :url="'supplier/' + supplierId + '/purchase-return'"
                          :filter="filter"
                      ...
                      <template v-slot:item.action="{ item }">
                        <v-menu offset-y transition="scroll-y-transition">
                          ...
                          <v-list class="actions">
                            ...
                            <v-list-item
                              @click="
                                $router.push('/purchase-return/edit/' + item.id)
                              "
                              link
                            >
                              <span>

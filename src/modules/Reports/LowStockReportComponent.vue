<template>
  <div>
    <PageTitle title="Low Stock Report" />
...
export default {
  ...
  methods: {
    generateReport() {
      ...
      this.$store
        .dispatch("report/getLowStockReports", filter)
        .then((res) => {
          this.lowStockList = res.data.data;
           let i = 1;
            this.lowStockList.map((p) => {
            p.no = i;
            i++;
          });
        })
    ...
    pdfdownload() {
      let Data = _.cloneDeep(this.lowStockList);
      ...
      doc.text("Low Stock Report", 15, 5);
      ...
      doc.save(`${moment(today).format("DD-MM-YYYY")}-LowStockReport.pdf`);
    },
    ...
    exceldownload() {
      ...
      XLSX.writeFile(
        wb,
        `${moment(today).format("DD-MM-YYYY")}-LowStockReport.xlsx`
      );
    },
  },
  computed: {
